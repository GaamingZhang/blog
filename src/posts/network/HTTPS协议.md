...
1. **获取证书**：客户端从服务器获取HTTPS证书（通常包含完整的证书链）
2. **验证证书有效期**：检查证书是否在有效期内（Not Before ≤ 当前时间 ≤ Not After）
3. **验证证书签名**：使用签发机构的公钥验证证书的数字签名，确保证书内容未被篡改
4. **验证证书链**：递归验证证书链中的每个证书，直到根证书（浏览器/操作系统预信任的CA证书）
5. **验证证书主题**：检查证书的主题（Subject）或扩展字段（Subject Alternative Name, SAN）是否与访问的域名匹配
6. **检查证书吊销状态**：通过以下机制之一检查证书是否被吊销：
   - **CRL（Certificate Revocation List）**：客户端下载CA发布的吊销列表，查找证书是否在列表中
   - **OCSP（Online Certificate Status Protocol）**：客户端向OCSP服务器查询单个证书的状态
   - **OCSP Stapling**：服务器预先从CA获取证书状态并附加在TLS握手中，减少客户端验证时间
   - **OCSP Must-Staple**：证书要求服务器必须提供OCSP Stapling信息，增强安全性

如果证书验证的任何一步失败，浏览器会显示不同级别的安全警告，提示用户网站可能存在安全风险，用户可以选择继续访问或终止连接。
...
- **握手速度更快**：从4-RTT减少到1-RTT，大幅降低连接建立延迟；支持0-RTT模式，允许客户端在首次请求时就发送加密数据
- **安全性更高**：移除了所有不安全的加密算法（如SHA-1、RC4、3DES、AES-CBC等）和特性（如压缩、NPN等）
- **加密套件更精简**：仅支持AEAD（Authenticated Encryption with Associated Data）加密套件，提供加密和认证一体化保障
- **密钥交换更安全**：强制使用前向安全的密钥交换算法（ECDHE/DHE），不支持RSA密钥交换
- **会话恢复机制改进**：使用PSK（Pre-Shared Key）代替Session ID，提高会话恢复的安全性和效率
- **扩展支持更完善**：支持更多安全扩展，如Certificate Transparency、Encrypted SNI等
