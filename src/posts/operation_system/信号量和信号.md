...
### Q1: 信号量和信号的核心区别是什么？
**A1**: 信号是进程间的异步通知机制，用于告知进程某个事件发生；信号量是进程/线程同步的计数器，用于控制共享资源访问。信号不阻塞发送方且可能丢失，信号量可能阻塞调用方且精确计数不丢失。

### Q2: 信号量的P操作和V操作分别是什么？
**A2**: 
- P操作（Wait/Down）：信号量值减1，如果结果小于0则阻塞等待
- V操作（Signal/Up）：信号量值加1，如果有进程/线程在等待则唤醒一个

### Q3: 二值信号量和计数信号量有什么区别？
**A3**: 
- 二值信号量：值只能为0或1，类似互斥锁，用于实现互斥访问
- 计数信号量：值可以为任意非负整数，用于控制有限资源的访问数量

### Q4: 为什么SIGKILL和SIGSTOP信号不能被捕获或忽略？
**A4**: 这两个信号是为了确保系统管理员能够可靠地终止或停止任何进程。如果允许捕获或忽略，可能导致恶意进程无法被终止，影响系统安全。

### Q5: 如何处理信号的丢失问题？
**A5**: 
- 使用实时信号（32-64）替代标准信号，实时信号支持队列机制，不会丢失
- 设计应用程序时避免依赖信号的重复发送
- 在信号处理函数中使用原子操作或锁机制确保数据一致性

### Q6: 信号量和互斥锁有什么关系？
**A6**: 二值信号量本质上就是互斥锁。互斥锁是信号量的一种特殊情况，当信号量的最大值为1时，就变成了互斥锁，用于保护临界区资源。

### Q7: 什么是信号的竞态条件？如何避免？
**A7**: 信号竞态条件是指在信号处理过程中，进程状态发生变化导致的不可预测行为。避免方法包括：
- 使用信号掩码保护临界区
- 在信号处理函数中使用异步信号安全的函数
- 使用sigaction()替代signal()设置信号处理函数

### Q8: 如何在Go语言中实现信号量？
**A8**: 在Go中可以使用带缓冲的channel实现信号量，例如：
```go
...
- full：控制已用缓冲区数量
- mutex：保护缓冲区的互斥访问
生产者先获取empty信号量，消费者先获取full信号量，确保缓冲区不溢出或下溢。

### Q10: 什么是信号处理函数的可重入性？
**A10**: 可重入性指信号处理函数在执行过程中再次被调用（如嵌套信号）时，能够正确处理而不导致数据不一致。信号处理函数应使用异步信号安全的函数，避免使用全局变量或需要加锁的资源。

### Q11: 系统V信号量和POSIX信号量有什么区别？
**A11**: 
- System V信号量：基于信号量集合，功能强大但使用复杂，支持原子操作多个信号量
- POSIX信号量：更简单易用，分为命名信号量（进程间）和无名信号量（线程间）

### Q12: 为什么信号量比忙等待更高效？
**A12**: 忙等待会占用CPU资源循环检查条件，而信号量的P操作会将进程置于等待队列，只有当资源可用时才被唤醒，避免了CPU资源浪费，提高了系统整体效率。

## 参考资源

- [Linux 信号文档](https://man7.org/linux/man-pages/man7/signal.7.html)
- [POSIX 信号量文档](https://man7.org/linux/man-pages/man7/sem_overview.7.html)
- [Linux 进程间通信文档](https://man7.org/linux/man-pages/man7/svipc.7.html)
