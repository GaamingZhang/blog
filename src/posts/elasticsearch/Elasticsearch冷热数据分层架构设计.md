...
> 关于 ES 写入流程与 Segment 机制，请参考《Elasticsearch写入查询流程与refresh_interval调优》。
...
`forcemerge` 在 Rollover 后将索引强制合并为单个 Segment，理由是已封存的索引不再写入，此时合并到 1 个 Segment 可以最大化查询性能，同时消除已标记删除文档的空间浪费。这个操作会消耗大量 I/O 和 CPU，建议通过调整 Warm 阶段的 `min_age`，让索引在业务低峰期进入 Warm 阶段时执行 forcemerge。
...
**Shrink**（收缩）将主分片数合并为 1，本质上是创建一个新的单分片索引，将原索引的所有段硬链接过去，因此几乎不产生额外磁盘 I/O。**readonly** 动作将索引设置为 `index.blocks.write: true`，防止意外写入。
...
`freeze` 动作会释放索引在内存中的数据结构（Field Data Cache 等），仅在查询时按需加载，极大降低内存占用。在 ES 8.x 中，更推荐的做法是在 Cold 阶段直接启用 **Searchable Snapshot**（后文详述），将数据迁移到对象存储，彻底释放本地磁盘空间。
...
Lucene 的文件结构天然友好于这种访问模式：查询时通常只需拉取与命中文档相关的少量文件块，不需要读取整个索引文件。
...
Hot 层配置 2～3 个主分片支撑并发写入，Warm 阶段通过 Shrink 合并为 1 个主分片：

```
Hot 层：  主分片 ×2，副本 ×1  → 共 4 个分片
Warm 层： 主分片 ×1，副本 ×1  → 共 2 个分片
Cold 层： 主分片 ×1，副本 ×0  → 共 1 个分片（Snapshot 保障可恢复性）
```

Cold 层副本数降为 0 是合理的——Searchable Snapshot 已将数据持久化到对象存储，节点故障时可重新挂载。
...
ILM 策略更新对已存在的索引不会立即生效，原因是每个索引会缓存其关联的策略版本。处理方式：等待下一个轮询周期（默认 10 分钟）自动生效，或对 ERROR 状态索引执行 `POST /logs-app-000001/_ilm/retry` 强制重试。

### Q2：Shrink 操作失败，提示分片未分配到同一节点

Shrink 要求所有主分片先迁移到同一个 Hot 节点才能完成文件硬链接操作。解决方案：通过 `index.routing.allocation.require._name` 将索引强制分配到指定节点，或在 ILM Warm 阶段的 `allocate` 动作中指定目标节点数为 1。

### Q3：Searchable Snapshot 查询超时或极慢

首次查询 Frozen 层索引时，对象存储的文件块尚未进入本地缓存，所有数据都需要从对象存储拉取，延迟可能达到数十秒。优化方向：预热缓存（业务低峰期提前发出探针查询）、调整 Shared Cache 大小（`xpack.searchable.snapshot.shared_cache.size`）、或改用 Full Copy 模式。

### Q4：跨层聚合查询变慢，如何隔离不同层的查询

跨层查询会同时命中 Hot、Warm、Cold 三层，Cold 层延迟会拖慢整体响应。解决方案：通过 `range` 查询限定时间范围，协调节点会跳过不匹配的 backing index；对跨层统计查询，使用 Rollup Job 或 Transform 提前固化聚合结果。

### Q5：如何监控冷热分层的运行状态和性能指标？

监控冷热分层架构需要关注三个维度的指标：

**分片迁移状态**：通过 `GET _ilm/explain` API 查看每个索引的 ILM 执行状态，包括当前阶段、待执行动作、上次执行时间等。重点关注处于 ERROR 状态的索引，及时排查迁移失败原因。

**各层节点资源使用**：Hot 层重点关注 CPU 使用率（写入压力）、JVM Heap 使用率（查询缓存）、磁盘 I/O 延迟；Warm/Cold 层重点关注磁盘空间增长趋势和查询延迟分布。可通过 `GET _nodes/stats` 获取详细指标，配合 Prometheus + Grafana 建立可视化监控面板。

**Searchable Snapshot 性能**：对于 Frozen 层，监控 Shared Cache 的命中率（`searchable_segments_cache.hit_count` 与 `miss_count`）、缓存驱逐率、对象存储请求延迟。命中率低于 80% 说明缓存配置可能不足，需要调整 `xpack.searchable.snapshot.shared_cache.size` 或增加节点内存。

建议配置告警规则：ILM 执行失败超过 10 分钟、Hot 层磁盘使用率超过 85%、Frozen 层首次查询延迟超过 30 秒时触发告警。
