...
# shard_num: 目标分片编号（从 0 开始）
# hash(): MurmurHash3 哈希函数，返回固定整数
# _routing: 路由键，默认为文档 _id
...
```
# 主分片数从 5 改为 10 后，同一文档的路由计算
shard_num = 12345678 % 10 = 8
# 结果：路由结果从分片 3 变成分片 8
# 问题：文档数据仍在分片 3，但查询会去分片 8 查找
```
...
# ceil(): 向上取整函数
# 预期总数据量: 根据数据保留策略估算的总量（如 6 个月）
# 单分片目标大小: 搜索场景取 30GB，日志场景取 50GB
# 增长系数: 建议 1.2 ～ 1.5，预留 20%～50% 增长空间
```
...
# 经验值：每 GB 堆内存最多承载 20 个分片（含主分片和副本）
# 超过此上限会导致 JVM GC 压力增大、元数据管理开销显著增加
```
...
1. **集群健康状态为 Green**：确保所有主分片和副本分片都已正确分配，无任何未分配分片。
2. **目标分片数必须是原分片数的整数倍**：如 5 分片只能拆分为 10、15、20 等，不能拆分为 7 或 12。
3. **原索引必须设置为只读**（`blocks.write: true`）。
4. **每个主分片只有一个副本分布在特定节点上**（副本数需先设为 0 或确保分片分布满足要求）。
...
以下是一个完整的 ILM 策略配置示例，实现日志索引的自动滚动与生命周期管理：
  ...
  "policy": {
    "phases": {
      "hot": {
        ...
        "actions": {
          "rollover": {
            "max_size": "50gb",      // 单索引达到 50GB 时滚动
            "max_age": "1d"          // 或超过 1 天时滚动
          },
          "set_priority": {
...
// 步骤 2：创建索引模板，关联 ILM 策略
PUT /_index_template/logs_template
{
  "index_patterns": ["logs-*"],
  "template": {
    "settings": {
      "index.lifecycle.name": "logs_policy",           // 关联 ILM 策略
      "index.lifecycle.rollover_alias": "logs-write",  // 滚动别名
      "number_of_shards": 1,                           // 初始主分片数
...
| 配置项 | 说明 |
|--------|------|
| `rollover.max_size` | 单索引大小上限，超过后自动创建新索引 |
| `rollover.max_age` | 索引最大存活时间，适合时间敏感数据 |
| `shrink.number_of_shards` | Warm 阶段将多分片合并为单分片，降低元数据开销 |
| `forcemerge.max_num_segments` | 合并 Segment 为 1，优化查询性能 |
| `is_write_index: true` | 标记当前活跃索引，写入请求通过别名路由到此索引 |
