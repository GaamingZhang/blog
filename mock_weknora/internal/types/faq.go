...
const (
	// FAQImportStatusPending 表示FAQ导入任务的待处理状态
	FAQImportStatusPending FAQImportTaskStatus = "pending"
	// FAQImportStatusProcessing 表示FAQ导入任务的处理中状态
	FAQImportStatusProcessing FAQImportTaskStatus = "processing"
	// FAQImportStatusCompleted 表示FAQ导入任务的已完成状态
	FAQImportStatusCompleted FAQImportTaskStatus = "completed"
	// FAQImportStatusFailed 表示FAQ导入任务的失败状态
	FAQImportStatusFailed FAQImportTaskStatus = "failed"
)

// FAQImportProgress 表示存储在Redis中的FAQ导入任务的进度
type FAQImportProgress struct {
	TaskID      string              `json:"task_id"`      // 导入任务的UUID
	KBID        string              `json:"kb_id"`        // 知识库ID
	KnowledgeID string              `json:"knowledge_id"` // FAQ知识ID
	Status      FAQImportTaskStatus `json:"status"`       // 任务状态
	Progress    int                 `json:"progress"`     // 0-100百分比
	Total       int                 `json:"total"`        // 要导入的总条目数
	Processed   int                 `json:"processed"`    // 已处理的条目数
	Message     string              `json:"message"`      // 状态消息
	Error       string              `json:"error"`        // 失败时的错误消息
	CreatedAt   int64               `json:"created_at"`   // 任务创建时间戳
	UpdatedAt   int64               `json:"updated_at"`   // 最后更新时间戳
}

// FAQImportMetadata 存储在Knowledge.Metadata中的FAQ导入任务信息
// Deprecated: 改用FAQImportProgress配合Redis存储
...
// ParseFAQImportMetadata 从Knowledge解析FAQ导入元数据。
